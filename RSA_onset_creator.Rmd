  ...
  for (j in 1:length(names)) {
   ...
   for (w in 1:length(word_list)) {
     ...
     x <- temp %>% 
      ...
      mutate(duration = 2) %>% 
      mutate(magnitude = 1)
     write.table(x, file = sprintf("RSA_3_column_onsets/%s/%s/%s_%s.txt", task, s_id, s_id, wd), append = FALSE, 
     ...
     y <- temp %>% 
      ...
      mutate(duration = 2) %>% 
      mutate(magnitude = 1)
     write.table(y, file = sprintf("RSA_3_column_onsets/%s/%s/%s_all_but_%s.txt", task, s_id, s_id, wd), append = FALSE, 
                 quote = FALSE, sep = " ",  eol = "\n", na = "NA", dec = ".", row.names = FALSE, col.names = FALSE, 
                 qmethod = c("escape", "double"))
   }
  }
...
for (i in 1:length(files)) {
  ...
  for (j in 1:length(names)) {
    ...
    temp <- file %>% 
      ...
      mutate(duration = 2) %>% 
      mutate(magnitude = 1)
    write.table(temp, file = sprintf("RSA_3_column_onsets/%s/others/not_%s.txt", task, s_id), append = FALSE, 
                quote = FALSE, sep = " ",  eol = "\n", na = "NA", dec = ".", row.names = FALSE, col.names = FALSE, 
                qmethod = c("escape", "double"))
  }
}
