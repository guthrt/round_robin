...
expName = 'fMRI'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\Taylor\\Downloads\\CSN Lab\\projects\\round_robin\\scripts\\scanner_portion\\psychopy_scripts\\runB\\runB_time.py',
...
intial_fix = visual.TextStim(win=win, name='intial_fix',
    text='+',
    font='Arial',
...
while continueRoutine:
    ...
    if t >= 0.0 and intial_fix.status == NOT_STARTED:
        ...
        intial_fix.tStart = t
        intial_fix.frameNStart = frameN  # exact frame index
        intial_fix.setAutoDraw(True)
    ...
    if t >= 0.0 and initial_trigger.status == NOT_STARTED:
        ...
        win.callOnFlip(initial_trigger.clock.reset)  # t=0 on next screen flip
...
trials = data.TrialHandler(nReps=1, method='sequential', 
    ...
    trialList=data.importConditions('runB.xlsx'),
...
for thisTrial in trials:
    ...
    # keep track of which components have finished
    trialComponents = [name, trait, fixation, response, trigger]
    ...
    trials.addData('onset', fixation.onset)
    thisExp.nextEntry()
